...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','organization.read','organization.write','warehouse.read'], 'user_A effective permissions = manager set');
select is((select array_agg(code || '|' || description order by code) from public.current_user_permissions()), array['branch.read|Read branches of own organization','organization.read|Read own organization','organization.write|Write own organization','warehouse.read|Read warehouses of own organization'], 'RPC returns code + description pairs');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','organization.read'], 'user_X effective permissions = cashier set');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['organization.read'], 'user_B effective permissions = operator set');
...
select is((select count(*)::int from public.current_user_permissions()), 6, 'admin gets all six permissions');
