...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','catalog.read','inventory.observe','inventory.read','layout.read','organization.read','sales.capture_own','sales.cedis_request','sales.quote','sales.read'], 'user_X effective permissions = cashier set');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['catalog.read','inventory.observe','inventory.read','layout.read','organization.read','sales.read'], 'user_B effective permissions = operator set');
...
select is((select count(*)::int from public.current_user_permissions()), 25, 'admin gets all twenty-five permissions (incl. catalog.* + inventory.* + layout.* + sales.*)');
