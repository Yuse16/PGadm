...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','catalog.read','inventory.observe','inventory.read','organization.read'], 'user_X effective permissions = cashier set');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['catalog.read','inventory.observe','inventory.read','organization.read'], 'user_B effective permissions = operator set');
...
select is((select count(*)::int from public.current_user_permissions()), 15, 'admin gets all fifteen permissions (incl. catalog.* + inventory.*)');
