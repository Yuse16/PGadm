...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','catalog.create','catalog.read','catalog.update','organization.read','organization.write','warehouse.read'], 'user_A effective permissions = manager set');
select is((select array_agg(code || '|' || description order by code) from public.current_user_permissions()), array['branch.read|Read branches of own organization','catalog.create|Create products, variants and barcodes','catalog.read|Read product master catalog','catalog.update|Edit catalog data and active/inactive transitions','organization.read|Read own organization','organization.write|Write own organization','warehouse.read|Read warehouses of own organization'], 'RPC returns code + description pairs');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['branch.read','catalog.read','organization.read'], 'user_X effective permissions = cashier set');
...
select is((select array_agg(code order by code) from public.current_user_permissions()), array['catalog.read','organization.read'], 'user_B effective permissions = operator set');
...
select is((select count(*)::int from public.current_user_permissions()), 11, 'admin gets all eleven permissions (incl. catalog.*)');
