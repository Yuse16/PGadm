...
select is((select count(*)::int from public.permissions), 15, 'admin reads the permission catalog incl. catalog.* + inventory.* (ACC-16)');
select is((select count(*)::int from public.role_permissions), 32, 'admin reads role_permissions of own orgs (no recursion)');
