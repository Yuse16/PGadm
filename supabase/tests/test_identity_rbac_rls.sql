...
select is((select count(*)::int from public.permissions), 25, 'admin reads the permission catalog incl. catalog.* + inventory.* + layout.* + sales.* (ACC-16)');
select is((select count(*)::int from public.role_permissions), 56, 'admin reads role_permissions of own orgs (incl. test legacy role, no recursion)');
