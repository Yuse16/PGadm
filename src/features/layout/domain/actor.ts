import { LayoutPermissionError } from "./layout-errors";

/**
 * Acting user for layout operations. Permission checks happen here, in the
...
export class LayoutActorImpl implements LayoutActor {
  readonly userId: string;
  readonly organizationId: string;
  readonly permissions: readonly string[];
  ...
  requirePermission(permissionCode: string): void {
    if (!this.hasPermission(permissionCode)) {
      throw new LayoutPermissionError(permissionCode);
    }
  }
}

export function createLayoutActor(input: {
  userId: string;
  organizationId: string;
  permissions: readonly string[];
}): LayoutActor {
  return new LayoutActorImpl(input);
}
