...
- **Status**: **1D.1 APROBADO + 1D.2 COMPLETED** — feature branch `feature/f1d-PG-INVENTORY-005-inventory` (base `ee761b1` HEAD 1C; HEAD `9e1bdf0`); sin PR, sin merge; pendiente revisión humana + integración a `develop`.
- **Scope**: Productos e inventario (Fase 2) — kickoff D-I01…D-I14 APPROVED (2026-08-06, `9bc1de3`); 1D.2 migración/seed/verificación DB.
...
- **Code**: `src/types/database.ts` regenerado con las 5 tablas + `_audit.inventory_events` (338 líneas). Port de integración de 1C.5 (`CatalogIntegrationRepository`) se puebla con stock real en 1D.5.
- **Tests**: `supabase/tests/test_inventory_stock.sql` (88) + suites ajustadas a 15 permisos / 35 role_permissions → **638/638 SQL** (10 archivos); **294/294 app**; `e2e:auth` 14/14; `e2e:identity` 39/39.
